...
class brand_message_page extends StatefulWidget {
  brand_message_page({Key? key, required this.brand_id, required this.brand_info, required this.user_info})
      : super(key: key);
  var brand_info, brand_id,user_info;

  @override
  State<brand_message_page> createState() => _brand_message_pageState();
}

class _brand_message_pageState extends State<brand_message_page> {
    ...
    if (isMoreData) {
      try {
        final collectionRef = await messages_docreference
            .doc("${widget.brand_id}+${widget.user_info["messaging_with"]}")
            .collection("chats")
  ...
  Widget build(BuildContext context) {
    ...
    sender.value = "${widget.brand_id}+${widget.user_info["messaging_with"]}";
    reciever.value = "${widget.user_info["messaging_with"]}+${widget.brand_id}";
    return Scaffold(
      body: SafeArea(
        child: Column(
          children: [
                                ...
                                Row(
                                  ...
                                  children: [
                                    ...
                                    Container(
                                        ...
                                        child: messageList[index]["message_type"]=="image"?
                                        Column(
                                            ...
                                            children: [
                                              CachedNetworkImage(
                                                ...
                                                imageBuilder: (context, url) =>
                                                    message_network_image_shape(url,wdt),
                                                placeholder: (context, url) =>
                                                    message_asset_image_shape(wdt),
                                                errorWidget: (context, url, error) =>
                                                    message_asset_image_shape(wdt),
                                              ).onTap(() {
                                                Get.to(() => view_full_image(),arguments: messageList[index]["message"]);
                                              }),
                                              2.heightBox,
                                              Text("${DateFormat('h:mm a').format(DateTime.fromMillisecondsSinceEpoch
                                                ((int.parse(messageList[index]["time"]) / 1000).toInt())
                                                  .toLocal())}",
                                                  style: TextStyle(
                                                      fontSize: 10,
                                                      color: darkFontGreyColor))
                                        ...
                                        Column(
                                            ...
                                            messageList[index]["sender"] ==
                                                widget.brand_id?
                                            Vx.yellow200:lightGreyColor)
                                            .padding(EdgeInsets.all(10))
                                            .rounded
                                            .make()
                                    ).box.margin(EdgeInsets.only(
                                        right: 15,
                                        top: 2.5,
                                        bottom: 2.5)).make()
                                  ],
                      ...
                      Material(
                        child: InkWell(
                          onTap: () async {
                            ...
                            send_message("text",message_txt_controller.text.trim());
                          },
                          borderRadius: BorderRadius.circular(50),
                          child: Ink(
                              height: 40,
  ...
  pick_image(context) async {
    ...
    Uint8List response = (await FlutterImageCompress.compressWithFile(img!.path,
          quality: 25))!;
  ...
  send_message(type,message_text) {
    ...
    sender_last_message_info = {
      "image": widget.user_info["image"],
      "name":widget.user_info["name"],
      "message": message_text,
      "message_type": type,
      "sender":widget.brand_id,
      "message_room_for":widget.brand_id,
      "messaging_with":widget.user_info["messaging_with"],
      "seen":"true",
      "time": message_time
    };
    ...
    reciever_last_message_info = {
      "image":widget.brand_info["brand_icon"],
      "name":widget.brand_info["brand_name"],
      "message": message_text,
      "message_type": type,
      "sender":widget.brand_id,
      "message_room_for":widget.user_info["messaging_with"],
